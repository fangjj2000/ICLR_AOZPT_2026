...
def interleave_datasets(dataset1, dataset2, L):
    ...
    while len(result) < total_len:
        result.extend(dataset1[i:i+L])
        result.extend(dataset2[i:i+L])
        i += L
    return result
class testDataset2(Dataset):
    def __init__(self, file_path_1, file_path_2, L):
        self.dataset_1 = pd.read_csv(file_path_1, sep='\t')
        prompts_1 = self.dataset_1['Prompt'].tolist()
        self.dataset_2 = pd.read_csv(file_path_2, sep='\t')
        prompts_2 = self.dataset_2['Prompt'].tolist()
        prompts = interleave_datasets(prompts_1, prompts_2, L)
        self.lines = [line.strip() for line in prompts]
    def __len__(self):
        return len(self.lines)
